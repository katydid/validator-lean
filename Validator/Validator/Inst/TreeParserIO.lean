...
abbrev Impl μ α [DecidableEq α] [Hashable α] β := StateT (State μ α) (EIO String) β
...
instance [DecidableEq α] [Hashable α]: Enter.DeriveEnter (Impl μ α) μ α where
  deriveEnter {ν: Nat} (xs: Rules μ (Pred α) ν): Impl μ α (IfExprs μ α (Symbol.nums xs)) := MemEnter.deriveEnter xs
...
instance [DecidableEq α] [Hashable α]: Leave.DeriveLeaveM (Impl μ α) μ α where
  deriveLeaveM {ν: Nat} (xs: Rules μ (Pred α) ν) (ns: List.Vector Bool (Symbol.nums xs)): Impl μ α (Rules μ (Pred α) ν) := MemLeave.deriveLeaveM xs ns

instance [DecidableEq α] [Hashable α]: ValidateM (Impl μ α) μ α where
  -- all instances have been created, so no implementations are required here

def run' [DecidableEq α] [Hashable α] (f: Impl μ α β) (t: Hedge.Node α): EIO String β :=
