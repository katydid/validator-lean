...
def denote {α: Type} {σ: Type} (Φ : σ -> List α -> Prop) (r: Regex σ): (xs: List α) -> Prop :=
  match r with
...
theorem denote_emptyset {α: Type} {σ: Type} (Φ: σ -> List α -> Prop):
  denote Φ emptyset = Language.emptyset := by
  simp only [denote]

theorem denote_emptystr {α: Type} {σ: Type} (Φ: σ -> List α -> Prop):
  denote Φ emptystr = Language.emptystr := by
  simp only [denote]

theorem denote_symbol {α: Type} {σ: Type} (Φ: σ -> List α -> Prop) (s: σ):
...
theorem denote_symbol' {α: Type} {σ: Type} (Φ: σ -> List α -> Prop) (s: σ):
  denote Φ (symbol s) = Φ s := by
  simp only [denote]

theorem denote_or {α: Type} {σ: Type} (Φ: σ -> List α -> Prop) (p q: Regex σ):
  denote Φ (or p q) = Language.or (denote Φ p) (denote Φ q) := by
  funext
  simp only [denote, Language.or]

theorem denote_concat_n {α: Type} {σ: Type} (Φ: σ -> List α -> Prop) (p q: Regex σ):
  denote Φ (concat p q) = Language.concat_n (denote Φ p) (denote Φ q) := by
  funext
  simp only [denote]

theorem denote_star_n' {α: Type} {σ: Type} (Φ: σ -> List α -> Prop) (r: Regex σ) (xs: List α):
