...
namespace EnvTreeParserStateWithMemTest
...
def TreeParserStateWithMemTest.mk (p: ParseTree.TreeParser): TreeParserAndMemTest :=
...
instance: MonadStateOf ParseTree.TreeParser TreeParserStateWithMemTest where
  get : TreeParserStateWithMemTest ParseTree.TreeParser := do
    let s <- EStateM.get
    return s.parser
  set : ParseTree.TreeParser → TreeParserStateWithMemTest PUnit :=
    fun parser => do
      let s <- EStateM.get
      EStateM.set (TreeParserAndMemTest.mk parser s.enter s.leave)
  modifyGet {α: Type}: (ParseTree.TreeParser → Prod α ParseTree.TreeParser) → TreeParserStateWithMemTest α :=
    fun f => do
      let s <- EStateM.get
...
instance
  ...
  [MonadStateOf ParseTree.TreeParser TreeParserStateWithMemTest] -- MonadStateOf ParseTree.TreeParser is declared above
...
def run (f: TreeParserStateWithMemTest α) (t: ParseTree): Except String α :=
  match EStateM.run f (TreeParserStateWithMemTest.mk (ParseTree.TreeParser.mk t)) with
...
def run' (f: TreeParserStateWithMemTest α) (t: ParseTree) (e: MemEnter.EnterMap) (l: MemLeave.LeaveMap): EStateM.Result String (MemEnter.EnterMap × MemLeave.LeaveMap) α :=
  match EStateM.run f (TreeParserAndMemTest.mk (ParseTree.TreeParser.mk t) e l) with
